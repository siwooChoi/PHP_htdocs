...
class ProductModel extends ExecuteModel {
  ...
  // 구매이력의 목록 보여주기
  public function showBuyList($user_name){
    $id_buylist = $user_name."_buylist";
    $productArray = array();

    $sql = "select * from $id_buylist order by b_Number desc";
    $productArray = $this->getAllRecord($sql);
  ...
  // 구매이력에 추가
  public function insertIntoBuyList($user_name, $amount){
    ...
    if($productAmount - $amount < 0){
      ...
      return "false";
        ...
        $sql = "insert into $id_buylist(p_Number, p_Name, p_Price, p_Comment, p_Type, p_Amount, p_Imgname, regist_day)
                        values(:p_number, :p_name, :p_price, :p_comment, :p_type, :p_amount, :p_imgname, :regist_day)";
  ...
  // 구매이력에서 삭제
  public function deleteBuyList($user_name_buylist, $pnum, $bnum){
    $sql = "delete from $user_name_buylist where p_Number = :pnum and b_Number = :bnum";
    $this->execute($sql, array(':pnum' => $pnum,
                               ':bnum' => $bnum)
                    );
  }
  ...
  // 회원탈퇴 시 구매이력 삭제
  public function deleteUserBuyList($user_name){
    $user_name_buylist = $user_name."_buylist";
    $sql = "drop table $user_name_buylist";
    $this->execute($sql);
  }
}
